...
extern crate nom;
extern crate serde;
extern crate serde_derive; // warnings

use colored::*;
use na::Matrix3;
use nom::{be_f32, call, error_position, le_f32, many_m_n, named_args};
...
impl VectorField {
    ...
    pub fn from_eigenanalysis(width: usize, height: usize, depth: usize, values: &[f32]) -> Self {
            ...
            for ez in (0..depth).rev() {
                ...
                for ex in 0..width {
                    ...
                    if (confidence - 1.0) < 0.01 {
                        ...
                        // fa value
                        fa = ((l1 - l2).powf(2.0) + (l1 - l3).powf(2.0) + (l2 - l3).powf(2.0))
                            .sqrt()
                            / (2.0 * (l1.powf(2.0) + l2.powf(2.0) + l3.powf(2.0))).sqrt();
                        if fa > FA_EPSILON {
                            // needle shaped tensor
        ...
        if largest + 6 < width * height * depth - 1 {
            eprintln!(
                "{} Largest index visited was {}, but total number of floats was {}.",
                "WARNING:".yellow().bold(),
                largest + 6,
                height * width * depth
            );
    ...
    pub fn calculate_seeding_points(
        &mut self,
        n_points: usize,
        step_size: usize,
        fa_area_threshold: f32,
    ) {
        ...
        for i in 0..n_points {
            ...
            if best.0 > 0.0 {
                ...
                println!(
                    "Found point with best = {:?} and fa = {}",
                    best, self.field[best.2 as usize][best.0 as usize][best.1 as usize].3
                );
...
impl Options {
    ...
    pub fn from_header_file(lines: impl IntoIterator<Item = impl AsRef<str>>) -> Self {
        ...
        Self {
            file: if file.components().count() == 0 {
                None
            } else {
                Some(file)
            },
